  ...
  const topPerformers = useMemo(() => {
    ...
    return activeOperators
      .map(o => {
        ...
        return { name: o.name, score: score, photoUrl: o.photoUrl };
      })
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="KPIs Computados" value={currentPeriodKPIs.length} subtitle={`Registros em ${MONTHS[Number(selectedMonth)-1]}/${selectedYear}`} icon={<Calendar size={24} />} color="blue" />
              ...
              topPerformers.map((op: any) => (
                <div key={op.name} className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold truncate max-w-[150px]">{op.name}</p>
                    <div className="w-full bg-gray-100 h-2 rounded-full mt-2">
                      <div className="bg-blue-600 h-2 rounded-full" style={{ width: `${op.score}%` }}></div>
                    </div>
                  </div>
                  <span className="text-sm font-bold text-blue-700">{formatDecimal(op.score)}</span>
                </div>
              ))
            )}
          </div>
